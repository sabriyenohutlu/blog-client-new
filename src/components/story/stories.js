...
const Stories = ({ latestStoryList, pinnedStoryList }) => {
  const options = {
    year: "numeric",
    month: "numeric",
    day: "numeric",
  ...
  return (
    <div className="container py-8">
      ...
      <section className="mb-16">
        <h1 className="text-4xl font-bold mb-8">Öne Çıkan Hikayeler</h1>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {pinnedStoryList?.map((story, index) => (
                  ...
                  <div className="absolute bottom-4 left-4 right-4 text-white">
                    <div className="flex items-center gap-2 text-sm mb-2">
                      <span className="px-2 py-1 bg-primary/20 rounded-full">
                        {story.story_category.map((category) => (
                          <span
                            key={category}
                ...
                <div className="p-6">
                  <div className="flex items-center gap-4 text-sm text-muted-foreground mb-4">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      {story.createdAt.toLocaleString("tr-TR", options)}
                    </div>
                  </div>
                  <p className="text-muted-foreground mb-4">
                    {story.story_summaryInfo}
                  </p>
      ...
      <section>
        <div className="flex items-center justify-between mb-8">
          <h2 className="text-3xl font-bold">Son Eklenen Hikayeler</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {latestStoryList?.map((story, index) => (
            <Card key={index} className="group hover-lift">
